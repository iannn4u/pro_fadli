    ...
    <div id="accordion-collapse" data-accordion="collapse">
        @foreach ($dailies as $date => $dailys)
            ...
            @foreach ($dailys as $daily => $value)
                ...
                <div id="accordion-collapse-body-{{ $value->id_daily }}" class="hidden overflow-x-auto"
                    aria-labelledby="accordion-collapse-heading-{{ $value->id_daily }}">
                    <table class="w-full text-sm text-left rtl:text-right dark:text-gray-400">
                        <tr class="bg-gray-100">
                            <th class="py-4 text-center">Nama Case</th>
                            <th class="py-4 text-center">Case</th>
                            <th class="py-4 text-center">Status</th>
                            <th class="py-4 text-center">Terakhir Update</th>
                            <th class="py-4 text-center relative">Aksi</th>
                            <th>
                                @if (auth()->user()->role == 'admin' || auth()->user()->role == 'supllier')
                                    <form action="/daily/{{ $value->id_daily }}" method="post" class="inline">
                                        @method('delete')
                                        @csrf
                            ...
                            @if ($value->lot->{'case' . $status + 1} === 'Iya')
                                <tr>
                                    ...
                                    <td class="px-6 py-3 text-center flex justify-center gap-5">
                                        <a href="/status/belumtersedia/{{ $statusValue->id_status }}"
                                        ...
                                        <a href="/status/terkirim/{{ $statusValue->id_status }}"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <a href="/status/pending/{{ $statusValue->id_status }}"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
