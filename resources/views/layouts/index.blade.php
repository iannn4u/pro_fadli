...
<body class="antialiased font-semibold">
    <div class="max-w-[2040px] mx-auto relative">

        @include('layouts.navbar')
        @include('layouts.sidebar')

        <div class="p-4 sm:ml-64">
            <div class="p-4 max-md:px-0 rounded-lg dark:border-gray-700 mt-14">
                @yield('main')
            </div>
        </div>
    </div>
    @if ($errors->any())
        <div class="fixed bottom-0 max-sm:right-2 md:right-5 p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50" role="alert">
            <div>
    ...
    @if (auth()->user()->name == null)
        <!-- Main modal -->
        <div class="fixed bg-black/30 bottom-0 top-0 right-0 left-0 z-40"></div>
        <div id="authentication-modal" tabindex="-1" aria-hidden="false"
            class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative top-[35%] left-[35%] p-4 w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 pt-0 md:p-5">
                        <form action="/update/name/{{ auth()->user()->id_user }}" method="post">
                            @method('put')
                            @csrf
                            <div class="mb-6">
                                <label for="name_user"
